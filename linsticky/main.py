...
class StickyApp(Adw.Application):
    """Main application class for LinSticky."""
    APP_INFO = load_app_info()
    ...
    def __init__(self):
        ...
        # Register signal handlers for graceful shutdown (Session Management)
    ...
    def do_activate(self):
        ...
        # Восстановление открытых заметок при запуске
        # Убедитесь, что метод называется именно так в вашем ApplicationManager
        if hasattr(self.app_manager, 'restore_notes'):
            self.app_manager.restore_notes()

    def _on_signal_quit(self, *args):
        """
        Callback for UNIX signals (SIGTERM, SIGINT).
        Ensures the application quits gracefully.
        """
        self.quit_app()
